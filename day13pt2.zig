...
fn count_tokens(input: []const u8, offset: i64) !i64 {
    var total: i64 = 0;
    ...
    while (num_strs.next()) |ax_str| {
        ...
        const tx = offset + try std.fmt.parseInt(i64, tx_str, 10);
        const ty = offset + try std.fmt.parseInt(i64, ty_str, 10);
...
pub fn main() !void {
    print("{d}\n", .{try count_tokens(@embedFile("input13.txt"), 10_000_000_000_000)});
}
...
test "example" {
    const input =
        ...
        \\
        ;
    try std.testing.expectEqual(480, count_tokens(input, 0));
}
