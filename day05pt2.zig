...
fn bad_pages(rules: *const Rules, pages: []u8) ?struct { usize, usize } {
    for (0..pages.len - 1) |b| {
...
pub fn main() !void {
    ...
    while (lines.next()) |line| {
        ...
        if (bad_pages(&rules, pages.items)) |_| {
            while (bad_pages(&rules, pages.items)) |bads| {
                const tmp = pages.items[bads[0]];
                pages.items[bads[0]] = pages.items[bads[1]];
                pages.items[bads[1]] = tmp;
            }
